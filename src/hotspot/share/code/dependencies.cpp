...
Klass* Dependencies::check_unique_implementor(Klass* ctxk, Klass* uniqk, KlassDepChange* changes) {
  ...
  InstanceKlass* ctx = InstanceKlass::cast(ctxk); // asserts is InstanceKlass
  assert(ctx->nof_implementors() > 0, "no implementors");
  if (ctx->nof_implementors() == 1) {
    assert(ctx->implementor() == uniqk, "sanity");
    return NULL;
  }
